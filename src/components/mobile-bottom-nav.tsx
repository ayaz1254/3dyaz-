...
export function MobileBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-white/10 bg-[#181a1b]/90 backdrop-blur-2xl md:hidden safe-area-bottom">
      <div className="flex items-center justify-around px-1 py-1">
          {tabs.map((tab) => {
          const href = tab.label === "Hesap" ? (session?.user ? "/dashboard" : "/login") : tab.href;
          ...
          const badge = tab.badge === "cart" ? cartCount : tab.badge === "wishlist" ? wishlistCount : 0;
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span className="absolute -top-0.5 left-1/2 h-0.5 w-6 -translate-x-1/2 rounded-full bg-[#05cc47]" />
              )}
            </Link>
          );
          })}
          {session?.user?.role === "admin" && (
            ...
            >
              ...
              {pathname.startsWith("/admin") && (
                <span className="absolute -top-0.5 left-1/2 h-0.5 w-6 -translate-x-1/2 rounded-full bg-amber-400" />
              )}
            </Link>
          )}
      </div>
